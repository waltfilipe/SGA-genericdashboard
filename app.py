...
# Modern cards always enabled (no sidebar toggle)
MODERN_CARDS = True
...
# Synthetic pass & defensive-action data for a generic player, 4 matches.
...
def _generate_generic_dataset(seed=_GEN_SEED, n_matches=N_GENERIC_MATCHES):
    ...
    for i in range(1, n_matches + 1):
        name = f"Match {i}"
        names.append(name)
        minutes[name] = float(rng.choice([90, 90, 90, 75, 60, 63, 65, 45]))
    base = {name: _gen_match_passes(rng) for name in names}
    deff = {name: _gen_match_def(rng) for name in names}
    return base, deff, minutes


BASE_MATCHES_DATA, DEFENSIVE_MATCHES_DATA, MATCH_MINUTES = _generate_generic_dataset(n_matches=N_GENERIC_MATCHES)
...
def _modern_card(title, border_color, items, comparison=False, blurred=False):
...
def section_card(title, border_color, items, blurred=False):
    if MODERN_CARDS:
...
defensive_all_stats = [
    compute_defensive_stats(defensive_dfs_by_match[m], m) for m in defensive_dfs_by_match
]
